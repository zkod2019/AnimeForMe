...
async function updateMangaList() {
  const myMangaRes = await fetch(
    `./ACM?username=${sessionStorage.getItem("userName")}&option=manga`
  );
    ...
    req.open(
      ...
      `https://api.jikan.moe/v4/manga?order_by=title&page=${currentPage}${
        mangaSearchInput.value
          ? `&q=${encodeURIComponent(mangaSearchInput.value)}`
          : ""
      }`,
      true
    );
  ...
  req.onload = function () {
    ...
    json.data.forEach((manga) => {
      topMangaAsListElements += `<li style="padding: 20px 20px 20px 20px;">
                    ...
                    <h4>${manga.title}</h4>
                    <button ${
                      myManga.data.find((a) => a.mangaId === manga.mal_id)
                        ? "disabled"
                        : ""
